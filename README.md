...
позволяет тестировать размер записей прямо из GUI.
...
`connect -> ClientHello -> server_hello_hmac_ok`.
...
отправки `ClientHello`. В `Браузерном`, `Тихом` и `Строгом` режимах такие
pre-ClientHello TCP-срывы включают короткую штрафную паузу уже для обычных и
медийных рукопожатий, а не только для скачивания/выгрузки. Это снижает шторм
повторных `connect()` по одному адресу прокси, но не смешивает сетевое
душение до TCP с JA4/ServerHello-ошибками.
...
- Расширить DRS из простого runtime-режима в фазовую модель: разные профили для
  ...
  повторяющихся шаблонов и сброс состояния после долгой паузы.
- Расширить IPT из простых неблокирующих пауз между TLS-записями в модель
  ...
  соединения, интерактивного трафика и передачи файлов.
