...
Зашит **реестр** браузерных профилей с выбором на лету, **«липким» на сессию** (настоящий браузер не меняет почерк между соединениями), а разные установки могут брать разные профили — отпечаток форка перестаёт быть одним общим маркером на всех. Сейчас в ротации — **проверенный Firefox-профиль** (JA4 `t13d1616h2_86a278354501_eeeea6562960`). Старый дефолтный ClientHello Telegram (`getDefault`) намеренно **исключён**: его JA4 — это в точности уже заблокированный отпечаток `t13d1516h2_8daaf6152771_d8a2da3f94cd` (проверено дампом). Добавлять новый профиль можно **только из реального перехвата** с подтверждённым JA4 — выдуманный отпечаток хуже одного правильного.
